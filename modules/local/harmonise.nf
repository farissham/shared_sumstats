process HARMONISE_SUMSTATS {
    ...
    input:
    tuple val(meta), path(sumstats), path(rsid_map)
    path snp_info
    ...
    script:
    // staged rsid_map (e.g. generated by LIFTOVER_PANEL) takes precedence over a
    // user-supplied meta.rsid_map path; [] means neither.
    def rsid_map_arg = rsid_map     ? "--rsid_map ${rsid_map}"      :
                       meta.rsid_map ? "--rsid_map ${meta.rsid_map}" : ""
